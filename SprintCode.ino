...
void updatePWMs(float totalDistanceLeft, float totalDistanceRight, float vL, float vR, float angleRad, float angleRadAccum, float error_left_accum, float error_right_accum, float theta_d) {
  ...
  /*leftMotorPWM =  kp*(jp*(((totalDistanceLeft+totalDistanceRight)/2)-angleRad) + ji*(-angleRadAccum)-vL) +ki*(error_left_accum);
  rightMotorPWM =  kp*(jp*(((totalDistanceLeft+totalDistanceRight)/2)-angleRad) + ji*(-angleRadAccum)-vL)+ki*(error_right_accum); */
  
 leftMotorPWM =   (kp*(jp*(theta_d-angleRad) + ji*(-angleRadAccum)-vL) +ki*(error_left_accum))*((vR-vL)+1);
 rightMotorPWM =  (kp*(jp*(theta_d-angleRad) + ji*(-angleRadAccum)-vL)+ki*(error_right_accum))*((vL-vR)+1);
  
  /* * kp = 300 ki = 5000 Jp = -6.25 Ji = -36
   * leftMotorPWM = 100+ 300*(-6.25*(-angleRad)-36*(-angleRadAccum)-vL) +5000*(error_left_accum);
  rightMotorPWM = 100+ 300*(-6.25*(-angleRad)-36*(-angleRadAccum)-vR)+5000*(error_right_accum);
  leftMotorPWM = 500*(-6*(-angleRad)-36*(angleRadAccum)-vL)+5000*(error_left_accum);
  rightMotorPWM = 500*(-6*(-angleRad)-36*(angleRadAccum)-vR)+5000*(error_right_accum);
  */
}
...
void loop()
{
  ...
  static float theta_d = 0;             // the desired angle
  static float c = -.581776;             // constant of bullshit
  static float m = .123271;
  ...
  if(shouldPrint)   // do the printing every 105 ms. Don't want to do it for an integer multiple of 10ms to not hog the processor
  {
        ...
        Serial.print(error_);
        Serial.print("\t");
        Serial.print(error_left_accum);
        Serial.print("\t");
        Serial.print(error_right_accum);
        Serial.print("\t");
...
/* Uncomment this and comment the above if doing wireless
  ...
  if(cur_time - prev_time > UPDATE_TIME_MS && start_flag)
  {
    ...
    //theta_d = (((totalDistanceLeft + totalDistanceRight)/2)*c)-m*((vL+vR)/2);
    theta_d = -m*((vL+vR)/2-.2);
    angle_rad_accum += (angle_rad - theta_d) *delta_t;
    error_left_accum += vL*delta_t;
    error_right_accum += vR*delta_t;
   
    

    updatePWMs(totalDistanceLeft, totalDistanceRight, vL, vR, angle_rad, angle_rad_accum, error_left_accum, error_right_accum, theta_d);
